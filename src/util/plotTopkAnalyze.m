function areaArr = plotTopkAnalyze(savepath, plotmode, topkData, spsMetric, covMetric, bmName, netNeurons)
    ...
    if plotmode == 1
        ...
        % 绘制每个不同的sps和cov下的图像，和random方法进行对比
        topkRateCell = {};
        figCell = {};
        x = (0:netNeurons)/netNeurons;
        areaTopkNum = {x(1:find(x == 0.2)), x};
        areaArr = zeros(numel(spsMetric), numel(covMetric)+1, numel(areaTopkNum));
              

        for sps = 1:numel(spsMetric)
            % 生成topkRateCell，cell的每一页都代表一个sps metric的数据，每一行代表cov metric的数据，每一列是不同的topk
            ...
            % 正常时候 'LineWidth', 1.5, 'MarkerSize', 5
            % 绘制legend时候使用'LineWidth', 3, 'MarkerSize', 15
            covCurveCell{1} = plot(x, ncData, '-*', 'Color', '#B22222', 'LineWidth', 3, 'MarkerSize', 15);
            hold('on')
        
            tkData = curveDataPage(:,:,2);
            covCurveCell{2} = plot(x, tkData, '-.square', 'Color', [0.48235 0.40784 0.93333], 'LineWidth', 3, 'MarkerSize', 15);
            hold('on')
        
            tncData = curveDataPage(:,:,3);
            covCurveCell{3} = plot(x, tncData, '--diamond', 'Color', [0.11765 0.56741 1], 'LineWidth', 3, 'MarkerSize', 15);
            hold('on')
        
            ttkData = curveDataPage(:,:,4);
            covCurveCell{4} = plot(x, ttkData, '-^', 'Color', '#2E8B57', 'LineWidth', 3, 'MarkerSize', 15);
            hold('on')
        
            pdData = curveDataPage(:,:,5);
            covCurveCell{5} = plot(x, pdData, '--o', 'Color', '#FFA500', 'LineWidth', 3, 'MarkerSize', 15);
            hold('on')
        
            ndData = curveDataPage(:,:,6);
            covCurveCell{6} = plot(x, ndData, '-.o', 'Color', '#DC143C', 'LineWidth', 3, 'MarkerSize', 15);
            hold('on')
        
            miData = curveDataPage(:,:,7);
            covCurveCell{7} = plot(x, miData, '-^', 'Color', '#8B4513', 'LineWidth', 3, 'MarkerSize', 15);
            hold('on')
        
            mdData = curveDataPage(:,:,8);
            covCurveCell{8} = plot(x, mdData, '-x', 'Color', '#696969', 'LineWidth', 3, 'MarkerSize', 15);
            hold('on')
        
            randData = curveDataPage(:,:,9);
            covCurveCell{9} = plot(x, randData, ':x', 'LineWidth', 3, 'MarkerSize', 15);
            hold('off')
            ...
            % % xticklabels(0:5:100);
            xlabel('% of neurons examined','FontSize',18);
            ylabel('detection rate','FontSize',18);
        
            % lgd = legend(covMetric{1}, covMetric{2}, covMetric{3}, covMetric{4}, covMetric{5}, covMetric{6}, covMetric{7}, covMetric{8}, 'random');
            lgd = legend('NC\qquad', 'TK\qquad', 'TNC\qquad', 'TTK\qquad', 'PD\qquad', 'ND\qquad', 'MI\qquad', 'MD\qquad', 'Random');
            lgd.Box = 'off';
            set(lgd,'FontName','Times New Roman','FontSize',18 ,'FontWeight','normal', ...
                'TextColor','black','orientation','horizontal', 'Interpreter', 'latex', ...
                'Location', 'northoutside');        %, 'NumColumns', 4
            lgd.Visible = "on";
            
            % figTitle = sprintf('Topk Comparison Suspiciousness Metric: %s', spsMetric{sps});
            title(bmName, 'FontSize', 18, 'FontWeight', 'bold')        
            
            % figFilename = sprintf('Topk Analyze figure Sps_%s', spsMetric{sps});
            figFilename = 'test';
            fig = gcf;
